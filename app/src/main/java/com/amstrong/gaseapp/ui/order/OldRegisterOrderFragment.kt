...
import com.amstrong.gaseapp.utils.toLocalDateTime
...
class OldRegisterOrderFragment : BaseFragment<MainViewModel, FragmentRegisterOrderBinding, MainRepository>() {

    companion object {
        fun newInstance() = OldRegisterOrderFragment()
    }
    ...
    private var startAt : Long = MaterialDatePicker.todayInUtcMilliseconds()
    private var endAt : Long = MaterialDatePicker.todayInUtcMilliseconds()

    //private lateinit var groupLines: ArrayList<LineItem>
...
//                    txtOrderOn.setText(order.purchase_date.formatToPattern("d MMM YYYY"))
//                    txtExpectedOn.setText(order.expected_date?.formatToPattern("d MMM YYYY"))
    ...
    private fun isOk() : Boolean{
        with(binding){
            if (txtReference.text.toString().isEmpty()){
                txtReference.setError(resources.getString(R.string.error_field_required))
            }

            if (txtSupplier.text.toString().isEmpty()){
                txtSupplier.setError(resources.getString(R.string.error_field_required))
            }

            if (txtStore.text.toString().isEmpty()){
                txtStore.setError(resources.getString(R.string.error_field_required))
            }

//            if (txtOrderOn.text.toString().isEmpty()){
//                txtOrderOn.setError(resources.getString(R.string.error_field_required))
//            }
//
//            if (txtExpectedOn.text.toString().isEmpty()){
//                txtExpectedOn.setError(resources.getString(R.string.error_field_required))
//            }

            return !(txtReference.text.toString().isEmpty() || txtSupplier.text.toString().isEmpty()  || txtStore.text.toString().isEmpty())
...
//        val formatter: DateFormat = SimpleDateFormat("d MMM yyyy")
...
//                purchase_date = formatter.parse(binding.txtOrderOn.text.toString())

//                expected_date = formatter.parse(binding.txtExpectedOn.text.toString())
